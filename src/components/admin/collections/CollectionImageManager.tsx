...
import {
  ...
  createCollectionImageUploadAction,
  deleteCollectionImageAction,
  reorderCollectionImagesAction,
  setCollectionImageRoleAction,
} from "@/server/actions/admin/collections";
import type { AdminCollection, CollectionImageRole } from "@/server/repositories/collections.repo";

const imageRoles: CollectionImageRole[] = ["primary", "gallery", "hero"];
...
export function CollectionImageManager({
  ...
  const [alt, setAlt] = useState(`${collection.name} collection image`);
  const [imageRole, setImageRole] = useState<CollectionImageRole>("gallery");
  ...
  async function handleUpload(event: FormEvent<HTMLFormElement>) {
    ...
    setIsUploading(true);
    const upload = await createCollectionImageUploadAction({
    ...
    const attached = await attachCollectionImageAction({
      alt,
      collectionId: collection.id,
      contentType: upload.contentType,
      imageRole,
      sizeBytes: file.size,
      sortOrder: collection.images.length,
  ...
  return (
    <section className="mt-12 border-t border-charcoal/10 pt-10">
      <div className="max-w-3xl">
        ...
        <h2 className="mt-3 font-serif text-3xl font-normal">Collection media</h2>
      ...
      <form className="mt-7 grid gap-5 border border-charcoal/10 bg-porcelain/72 p-5 lg:grid-cols-[1fr_1fr_auto]" onSubmit={handleUpload}>
        <div>
          <label className="mb-2 block text-[0.65rem] uppercase tracking-[0.18em] text-charcoal/55" htmlFor="collection-image">
            Image
          </label>
          <input accept={acceptedTypes.join(",")} className="form-control text-[0.9rem]" disabled={!isUploadConfigured} id="collection-image" name="image" type="file" />
        </div>
        <div>
          <label className="mb-2 block text-[0.65rem] uppercase tracking-[0.18em] text-charcoal/55" htmlFor="collection-alt">
            Alt text
          </label>
          <input className="form-control text-[0.9rem]" disabled={!isUploadConfigured} id="collection-alt" name="alt" onChange={(event) => setAlt(event.target.value)} value={alt} />
        </div>
        <div>
          <label className="mb-2 block text-[0.65rem] uppercase tracking-[0.18em] text-charcoal/55" htmlFor="collection-image-role">
            Role
          </label>
          <select
            className="form-control text-[0.9rem]"
            disabled={!isUploadConfigured}
            id="collection-image-role"
            name="imageRole"
            onChange={(event) => setImageRole(event.target.value as CollectionImageRole)}
            value={imageRole}
          >
            {imageRoles.map((role) => (
            ...
            <article className="grid gap-4 border border-charcoal/10 bg-porcelain/64 p-4 sm:grid-cols-[8rem_1fr]" key={image.id}>
              ...
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {imageRoles.map((role) => (
                    <form action={setCollectionImageRoleAction} key={role}>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <form action={reorderCollectionImagesAction} className="flex gap-2">
                    <input name="collectionId" type="hidden" value={collection.id} />
                    <input name="imageId" type="hidden" value={image.id} />
                  ...
                  <form action={deleteCollectionImageAction}>
                    <input name="collectionId" type="hidden" value={collection.id} />
                    <input name="imageId" type="hidden" value={image.id} />
