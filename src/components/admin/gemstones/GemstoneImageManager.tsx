...
export function GemstoneImageManager({
  ...
  const [alt, setAlt] = useState(`${gemstone.name} gemstone image`);
  const [imageRole, setImageRole] = useState<GemstoneImageRole>("gallery");
  ...
  async function handleUpload(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsUploading(true);
    const upload = await createGemstoneImageUploadAction({
    ...
    const attached = await attachGemstoneImageAction({
      alt,
      contentType: upload.contentType,
      gemstoneId: gemstone.id,
      imageRole,
      sizeBytes: file.size,
      sortOrder: gemstone.images.length,
  ...
  return (
    <section className="mt-12 border-t border-charcoal/10 pt-10">
      <div className="max-w-3xl">
        ...
        <h2 className="mt-3 font-serif text-3xl font-normal">Gemstone media</h2>
      ...
      <form className="mt-7 grid gap-5 border border-charcoal/10 bg-porcelain/72 p-5 lg:grid-cols-[1fr_1fr_auto]" onSubmit={handleUpload}>
        <div>
          <label className="mb-2 block text-[0.65rem] uppercase tracking-[0.18em] text-charcoal/55" htmlFor="image">
            Image
          </label>
          <input accept={acceptedTypes.join(",")} className="form-control text-[0.9rem]" disabled={!isUploadConfigured} id="image" name="image" type="file" />
        </div>
        <div>
          <label className="mb-2 block text-[0.65rem] uppercase tracking-[0.18em] text-charcoal/55" htmlFor="alt">
            Alt text
          </label>
          <input className="form-control text-[0.9rem]" disabled={!isUploadConfigured} id="alt" name="alt" onChange={(event) => setAlt(event.target.value)} value={alt} />
        </div>
        <div>
            ...
            <article className="grid gap-4 border border-charcoal/10 bg-porcelain/64 p-4 sm:grid-cols-[8rem_1fr]" key={image.id}>
              ...
              <div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {imageRoles.map((role) => (
                    <form action={setGemstoneImageRoleAction} key={role}>
                      <input name="gemstoneId" type="hidden" value={gemstone.id} />
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <form action={reorderGemstoneImagesAction} className="flex gap-2">
                    <input name="gemstoneId" type="hidden" value={gemstone.id} />
                    <input name="imageId" type="hidden" value={image.id} />
                  ...
                  <form action={deleteGemstoneImageAction}>
                    <input name="gemstoneId" type="hidden" value={gemstone.id} />
                    <input name="imageId" type="hidden" value={image.id} />
